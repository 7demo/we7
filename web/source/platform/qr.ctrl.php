...
if($do == 'del') {
    if ($_GPC['scgq']) {
        $list = pdo_fetchall("SELECT id FROM ".tablename('qrcode')." WHERE uniacid = '{$_W['uniacid']}' AND status = '0'", array(), 'id');
        if (!empty($list)) {
...
if($do == 'display') {
    ...
    $pindex = max(1, intval($_GPC['page']));
    $psize = 10;
    $list = pdo_fetchall("SELECT * FROM ".tablename('qrcode_stat')." WHERE uniacid = '{$_W['uniacid']}' $where ORDER BY createtime DESC LIMIT ".($pindex - 1) * $psize.','. $psize);
    if (!empty($list)) {
        foreach ($list as $index => &$qrcode) {
            if ($qrcode['type'] == 1) {
                $qrcode['type']="关注";
