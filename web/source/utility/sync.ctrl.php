...
if(!empty($_W['uniacid'])) {
	...
	$acids = pdo_fetchall('SELECT acid, uniacid FROM ' . tablename('account_wechats') . ' WHERE uniacid = :uniacid AND (level > 2)', array(':uniacid' => $_W['uniacid']));
	if(!empty($acids)) {
		foreach($acids as $acid) {
			$data = pdo_fetchall('SELECT fanid, openid, acid, uid, uniacid FROM ' . tablename('mc_mapping_fans') . " WHERE uniacid = :uniacid AND acid = :acid AND follow = 1 ORDER BY updatetime ASC, fanid DESC LIMIT 10", array(':uniacid' => $_W['uniacid'], ':acid' => $acid['acid']));
			if(!empty($data)) {
				$acc = WeAccount::create($acid['acid']);
				foreach($data as $row) {
